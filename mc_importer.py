...
"""
Temporary experiments for scraper and importer testing.
"""
...
def getty_scrape(INC_SIZE = 100,
    ...
    for c,xids in enumerate(group(ids, INC_SIZE)):
        ...
        assert (len(hh['images']) + len(hh['images_not_found'])) == input_size,(len(hh['images']),
...
def getty_import(dd = 'getty/json/images/',
                 index_name = 'getty_test',
                 doc_type = 'image',
                 ):
    """
    Import scraped Getty JSON -> ES.
    """
        ...
        for dir_name, subdir_list, file_list in walk(dd):
            ...
            for fn in file_list:
                ...
                hh = {'_id':'getty_' + h['id'],
                      ...
                      #'dims_thumb':[{'width':x['width'],'height':x['height']} for x in h['display_sizes'] if x['name'] == 'thumb'][0]
                      }
    ...
    for is_success,res in parallel_bulk(es,
        ...
        #FORMAT: (True, {u'index': {u'status': 201, u'_type': u'image', u'_shards': {u'successful': 1, u'failed': 0, u'total': 1}, u'_index': u'getty_test', u'_version': 1, u'_id': u'getty_100113781'}})
        pass
